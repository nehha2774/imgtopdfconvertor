    ...
    def convert_images_to_pdf(self):
        ...
        try:
            ...
            for image_path in self.image_paths:
                ...
                pdf.setFillColor(colors.white)  # Set fill color to white
                pdf.rect(0, 0, 612, 792, fill=True)
                pdf.drawInlineImage(img, x_centered, y_centered, width=new_width, height=new_height)
                pdf.showPage()  # Add this parentheses to correctly call the method

            pdf.save()
            messagebox.showinfo("Success", f"PDF has been saved successfully as '{output_pdf_path}'")
            self.image_paths=[]
            self.selected_images_listbox.delete(0,tk.END)
